...
<jsp:include page="../views_ext/header.jsp">
	<jsp:param name="title" value="View Catalogue" />
</jsp:include>
<hr />
<div class="container">
	<div class="span12">
		...
		<c:if test="${empty documents}">
			<p>No items exist in the catalogue.</p>
		</c:if>
		<table class="table table-hover">
			<caption></caption>
			<thead>
				<tr>
					<th><a href="/catalogue?sort=title"> Title </a></th>
					<th><a href="/catalogue?sort=type"> Type </a></th>
					<th><a href="/catalogue?sort=category"> Category </a></th>
					<th><a href="/catalogue?sort=publisher"> Publisher </a></th>
					<th><a href="/catalogue?sort=quantity"> Quantity </a></th>
				</tr>
			</thead>
			<tbody>
					...
					<tr>
						<td><c:choose>
								<c:when test="${current.type == 'Book'}">
									<a href="/book/<c:out value="${current.ISBN} "/>"><c:out
											value="${current.title} " /> <c:choose>
										...
										</c:choose> <c:out value=" Edition, ${current.year})" /> </a>
									<br />
											Author(s): - 
											<c:forEach items="${current.authorNames}" var="curAuthor">
